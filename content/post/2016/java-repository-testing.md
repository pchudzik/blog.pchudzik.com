...
A few days ago I’ve stumbled upon SQL query performance issue. Git claims that I was the author so
...
original query became too slow.

When working on query optimization I was really happy to find that detailed tests are in place. With
...
We are talking about the spring-boot application with hibernate and flyway. The assumption is that
...
Now we have tests which will fail in case of query change. It is safe to refactor and optimize. We
are also future proof in case of any DB or model change we will know from CI tool when something is
wrong. What's more, with few additional steps we will be able to make sure everything will work on
the production like DB.
...
Note that by default we are running with dev profile and we load flyway migrations from the property
named db.migrations.
...
with both databases. There is more. On dev environment you can load sample data to fill up the
database with something which will make the application look like alive system.
...
Postgres: ```./gradlew test -Dspring.profiles.active=postgres,test``` On localhost you will be able
to work fast (applying 100+ migrations to in memory H2 is faster than working with traditional DB)
and your CI environment will handle testing application on the production like DB.
